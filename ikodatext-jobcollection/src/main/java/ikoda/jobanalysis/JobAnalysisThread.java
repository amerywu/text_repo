...
public class JobAnalysisThread extends Thread {
	...
	private List<String> logIgnoreList = new ArrayList<>(
			Arrays.asList("Netio 02", "Low Call Count", "Excessive Accumulated Calls"));
	...
	public void runTextProcesses() throws JAException {
		try {
			...
			if (config.isSendToES()) {
				...
				ElasticSearchManager.getInstance().createIndexIfNotExisting(EsJson.JOBS_INDEX_NAME,
						EsJson.jobsIndexJson());
				ElasticSearchManager.getInstance().createIndexIfNotExisting(EsJson.POSSIBLE_PHRASES_INDEX_NAME,
						EsJson.possiblePhrasesIndexJson());
